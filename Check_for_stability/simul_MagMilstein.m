...
%   Ver. 2 (30-Aug-2022)
%     1) seed=5489 and rng(seed,'twister') were commented out. 
%        Thus, they should be set before simul_MagMilstein.m starts.
%%%%%
%   Ver. 1 (3-Jun-2022)
%
% This was copied from simul_Milstein. (Ver. 0), and 
% was revised for the Magnus-type Milstein method.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while (eps<abs(Tend-tpoint))
    ...
    for i=1:traject % starts from 1
        ...
        l=1; % note l=j:wdimM1
        normA12=normA12-tranij(tmp_ibase_index+l);
        %
        j=2;
        tmp_ibase_index=ibase_index+(j-1)*wdimM1; % note j-1
        l=1; % note l=1:j-1
        normA12=normA12+tranij(tmp_ibase_index+l);
        %
        tmpMatOm2=matTilF0*step+...
            matF1*rani(ibase+1)*sqstep+matF2*rani(ibase+2)*sqstep+...
            matG*normA12*step;
        tmpRm=expm(tmpMatOm2);
        tmpVec=tmpRm*[yVec(1,i) yVec(2,i)]';
        %%%%%
        yVec(1,i)=tmpVec(1);
        yVec(2,i)=tmpVec(2);
